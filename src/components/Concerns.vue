<template>
    <div style="position:relative">
        <div v-if="concerns_message.length<=0" style="padding-top:100px;color:gray;text-align:center;width:520px;">
            还没有关注用户！快去关注吧~
        </div>
        <div v-else style="float:left">
            <div style="width:520px;border-top-left-radius:12px;border-top-right-radius:12px;padding:12px 0 12px 24px;background:white">全部关注</div>
            <div  v-bind:key="index" v-for="(site,index) in concerns_message">
                <div class="concerns_fans_message" >
                    ...
                    <span v-if="site.follow_status!=site.be_follow_status&&site.follow_status==1" class="action" @click='follow_author(0,index,1)'>已关注</span>
                    <span v-else-if="site.follow_status=site.be_follow_status&&site.follow_status==1" class="action"  @click='follow_author(0,index,1)'>已互关</span>
                    <span v-else-if="site.follow_status!=site.be_follow_status&&site.follow_status!=1" class="action"  @click='follow_author(1,index,1)'>+关注</span>
                    <span v-else class="action"  @click='follow_author(1,index,1)'>+关注</span>
                </div>
            </div>
        </div>
...
export default {
    ...
    */
        get_concerns(){
            this.$axios.get('/api/follow_list',{
                headers: {
